...
exports.getBasicInfo = async(id, options) => {
  ...
  // console.dir( info , { showHidden: false, depth: 4, colors: true });
...
const getHTML5player = body => {
  let html5playerRes =
    /<script\s+src="([^"]+)"(?:\s+type="text\/javascript")?\s+name="player_ias\/base"\s*>|"jsUrl":"([^"]+)"/
      .exec(body);
  const htmlPlayer =  html5playerRes ? html5playerRes[1] || html5playerRes[2] : null;
  if(!htmlPlayer && global && global.reportLog) 
      global.reportLog('html5 player Error',body)
  return htmlPlayer;
};
...
const parseJSON = (source, varName, json) => {
  if (!json || typeof json === 'object') {
    ...
    try {
      ...
      if( global && global.reportLog) 
        global.reportLog(varName,source, err.message ,json );
...
const getVideoInfoPageEmbed = async(id, options) => {
  ...
  //   },
  // });
  let body = await miniget("https://www.youtube.com/get_video_info?el=embedded&hl=en&ps=default&video_id="+ id, options.requestOptions).text();
  let info = querystring.parse(body);
  info.player_response = findPlayerResponse('get_video_info_embed `player_response`', info);
  return info;
};
