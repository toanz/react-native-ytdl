...
exports.getBasicInfo = async(id, options) => {
  ...
  let player_response = await retryFn(getVideoInfoPageEmbed, [id, options, ''], retryOptions);
...
const getVideoInfoPageEmbed = async(id, options, info) => {
  const url = urllib.format({
    protocol: 'https',
    host: INFO_HOST,
    pathname: INFO_PATH,
  ...
  let morebody = await miniget("https://www.youtube.com/get_video_info?el=embedded&hl=en&ps=default&video_id="+ id, options.requestOptions).text();
  let moreinfo = querystring.parse(morebody);
  return parseJSON('get_video_info `player_response`', moreinfo.player_response || info.playerResponse);
};
